...
- Verify the Debrief Summary presents player-facing identity only: no raw Capture ID occupies a row (it appears in the header tooltip), the recorded Boss name accompanies its HP percentage or an explicit unavailable state, and every death shows its recorded Job abbreviation rather than the capture-time `Player N` alias, falling back to that alias only for an unresolved Job. One recorded death must render a single row with no repeated sequence block; two or more must group on the same five-second predicate as Death Quick Jump, so a full-party Wipe collapses into one Job-icon row with the derived count title and timestamp range. The Wipe row must appear whenever a Wipe timestamp is recorded, and the suggested-window Replay button must be the default item so the Summary is confirmable without aiming at it.
