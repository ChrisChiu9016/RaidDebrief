...
- The Action Effect name fix passed fresh `RaidDebrief.Core.Tests` 99／99, `RaidDebrief.UI.Tests` 4／4, and `RaidDebrief.Plugin.Tests` 174／174 with 0 failures; `dotnet format RaidDebrief.sln --verify-no-changes --no-restore` also passed. The fresh x64 Debug solution build completed with no warnings or errors. Its configured 1,000,448-byte DLL had `LastWriteTime` 2026-08-15 00:32:09.3410494 ＋08:00 and SHA-256 `89ee8bc3237f8c47dc5d5db277a890a8ce983267a1c18bd8815b66418fe22044`; Dalamud logged unload at 00:32:09.979, load at 00:32:10.014, and successful plugin-window initialization at 00:32:10.487.
- Multi-enemy mitigation passed fresh `RaidDebrief.Core.Tests` 99／99, `RaidDebrief.UI.Tests` 4／4, and `RaidDebrief.Plugin.Tests` 174／174 with 0 failures; the focused `ReplayEventNavigationTests` passed 21／21 and `dotnet format RaidDebrief.sln --verify-no-changes --no-restore` passed. The fresh x64 Debug build completed with no warnings or errors. Its configured 1,000,448-byte DLL had `LastWriteTime` 2026-08-15 01:14:34.3933228 ＋08:00 and SHA-256 `9b68b7919ebe67fba0bfda3122b11c55ce92803d3f9a9efae701771409f6fff8`; Dalamud logged unload at 01:14:34.964, load at 01:14:34.997, and successful plugin-window initialization at 01:14:35.406.
...
- At 00:02:44.921 on 2026-08-15, Runtime Replay loaded current-build `LastCompletedPull` `0619f062-21aa-44d7-aa6d-f9727f614059` with 629 Frames and 96 Events on Map 808. User-reported manual checks passed Play／Pause, scrubbing, ±1-second seek, end playback, Player／Boss position continuity, selected-player Focus and drag cancellation, Boss HP／Cast with readable Action names, and Death Timeline hover／Quick Jump with paused arrival. At Replay timestamp 00:47, The Blackest Night was present: the Party HP row showed its left-anchored barrier strip, Actor Context showed a nonzero Barrier value／percentage, and the status icon and remaining time updated correctly. This accepts current-build Barrier／Status presentation for this representative case, but does not inspect the raw recorded values or other status families.

- Current-build Developer／Test Capture `1661a50a-7961-4b95-a0c7-c9aca5e85c2d` (SHA-256 `74988a4c95d8328dca68b1adb5523ea7b6e40351b1fa45c3e2748dd771b65e4f`) recorded from 00:11:44.011 to 00:12:10.114 ＋08:00 in Territory 1154／Map 835. It finalized and validated with 262 Frames, 12 Events, and 17 Action Effects, exported in 41.83 ms, then loaded successfully from both Runtime `LastCompletedPull` and the exact JSON fixture. Automatic Capture was re-enabled afterward.
- Raw inspection found all Capture-side feature flags expected for this solo scenario: `Current`, Cast／Status timing, Action Effects, Action-name snapshots, and Barrier state. Frame timestamps were strictly increasing from 1 to 26,100 ms with 96–104 ms intervals averaging 99.996 ms; no Frame contained duplicate StableActorIds. The DRK Player and targetable Boss were present in all 262 Frames, and the Player recorded 99 distinct positions. Because this was a solo Capture, Party-membership and eight-player continuity were not exercised.
- The Blackest Night StatusId `1178` and Barrier were observation-aligned: the Player changed from 0% Barrier to 24% at 9,503 ms with StatusGained and 6.8555 seconds remaining, dropped to 11% at 14,502 ms after absorption, then returned to 0% at 16,402 ms with StatusLost and 0.0568 seconds remaining. Both Boss CastStarted Action IDs had sorted, unique Japanese StaticExcel snapshots (`33540` `オン・ザ・ソウル`; `33517` `パラデイグマ`), with no missing, extra, RSV, numeric, or blank names. Current Offline verification replayed all data; 20,000 deterministic seeks averaged 0.01114 ms with 0 bytes allocated per seek.
- User-verified Replay of a temporary legacy-data regression fixture first displayed ActionId `34423` as localized `オートアタック` in Last Hits, then as the killing blow after the fixture reassigned the actual lethal Action Effect. The fixture intentionally contained no recorded Action-name snapshot for `34423`; this directly exercises Replay's current-game-data Auto-attack category fallback instead of a prefilled label.
- Current-build manual Capture `0a7120bb-5d4d-4887-ae30-0501ead6209f` (SHA-256 `77e7707945830ba420a02d54a4095dac7d5101c0abbf7391088977c043750523`) ran from 00:43:54.696 to 00:44:06.324 ＋08:00 in Territory 1088／Map 808. It finalized and validated with 117 Frames, 4 Events, and 4 Action Effects, then exported in 11.46 ms. Three ActionId `31047` effects came from the Boss `ヘファイストス` to the Player at 2,206／5,231／8,255 ms, including 22,932 and 23,274 damage; no `CastStarted` event existed for that Action ID. Capture nevertheless stored exactly one `{ actionId: 31047, name: "オートアタック", language: "Japanese", source: "staticExcel" }` snapshot, closing the reported non-cast ordinary-attack naming gap. Offline verification replayed all 117 Frames and completed 20,000 deterministic seeks at 0.01163525 ms average with 0 bytes allocated per seek.
- Automatic Capture, temporarily disabled for the manual test, was restored afterward; the persisted setting reads `AutomaticCaptureEnabled: true` and Dalamud logged enablement at 00:47:57.347.
- Fresh automatic Runtime Pull `b5324338-aa3d-4b0c-afc3-2db427da2fcf` started at 00:54:12.281 in Territory 949／Map 636, finalized and validated after `DutyWiped` at 00:54:27.839 with 156 Frames, 9 Events, and 7 Action Effects, then loaded from the exact in-memory `LastCompletedPull` at 00:54:30.235. The user confirmed its ordinary attack was displayed correctly in Replay. This accepts the complete current-build Automatic Capture → Runtime handoff → ordinary-attack presentation path without JSON.
- A temporary dual-Boss regression fixture derived from recorded Capture `4de8c720-03b4-4ab4-8d55-e6a97378e0ab` preserved three concurrent target-aware Feint StatusId `1195` observations at 00:06.5 and made their three recorded enemies concurrently targetable; two names were disambiguated as `リンドブルム A`／`リンドブルム B`, while the third remained `リンドブルム`. Offline verification replayed all 4,066 Frames and completed 20,000 deterministic seeks at 0.025849585 ms average with 0 bytes allocated per seek. On the hot-reloaded DLL, the user observed all three enemy-targeted entries in `ACTIVE MITIGATION / HoT` and the three corresponding target names, proving the UI no longer collapses concurrent enemy debuffs to one primary Boss.


### Remaining current-build acceptance

- Post-reload automatic Pull `66043fb5-d46b-48ba-9048-a25f53190b2a` started at 22:19:30.835 in Territory 1363／Map 79, finalized after `DutyWiped` at 22:33:24.844 with 8,341 frames, 14,156 events, and 6,262 Action Effects, and completed validation at 22:33:24.890. Runtime Replay loaded that exact CaptureId at 22:36:01.728 with the same 8,341 frames and 14,156 events, proving current-build finalization, publication, and in-memory handoff without JSON. Its Pull-specific raw output is no longer available, but the representative current-build manual Capture above now verifies Barrier, Action-name, Actor, and sampling output contracts.
...
- The long current-build Pull isolated one volatile ObjectTable Actor `NullReferenceException` at slot 15 and continued to successful finalization; a pre-final-build Pull similarly isolated one slot 71 failure. The per-Actor containment path is observed, and the representative current-build manual Capture has complete Player／Boss continuity; continuity around the long Pull's isolated slot-15 failure cannot be inspected because that Runtime-only Pull was superseded.
...
- Phase 4 still requires the documented real `DutyWiped` scenario with multiple player deaths, resolvable Boss HP, and a raise followed by repeated death, followed by exact Summary → suggested-start Replay and next-Pull non-interference. Phase 5 current-build acceptance now covers playback, Actor／Focus, Boss HP／Cast, Death Timeline／Quick Jump, representative Barrier／Status presentation, and corresponding raw output; other status families and the complete visual surface remain to be accepted.
