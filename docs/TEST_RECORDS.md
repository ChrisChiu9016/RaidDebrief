...
- At 00:02:44.921 on 2026-08-15, Runtime Replay loaded current-build `LastCompletedPull` `0619f062-21aa-44d7-aa6d-f9727f614059` with 629 Frames and 96 Events on Map 808. User-reported manual checks passed Play／Pause, scrubbing, ±1-second seek, end playback, Player／Boss position continuity, selected-player Focus and drag cancellation, Boss HP／Cast with readable Action names, and Death Timeline hover／Quick Jump with paused arrival. This Pull had no confirmable Barrier／Status segment, so it does not accept those outputs.
